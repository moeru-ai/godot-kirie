...
function forwardPointerEvent(event: PointerEvent, phase = pointerPhase(event.type)): void {
...
export function installPointerEventForwarding(): () => void {
  ...
  const handlePointerDown = (event: Event): void => {
    ...
    forwardPointerEvent(pointerEvent);
  ...
  const handlePointerHover = (event: Event): void => {
    ...
    forwardPointerEvent(event as PointerEvent);
  };
  ...
  const removePointerEventForwarding = (): void => {
    ...
    for (const pointerEvent of claimedPointers.values()) {
      forwardPointerEvent(pointerEvent, "cancel");
    }
